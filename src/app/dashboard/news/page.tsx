  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-3 rounded-2xl border border-blue-500/40 bg-blue-500/10 px-5 py-3 shadow-[0_0_30px_rgba(59,130,246,0.28)]">
            <Newspaper className="w-8 h-8 text-blue-400" />
            <h1 className="text-3xl sm:text-4xl font-black text-foreground tracking-tight">News</h1>
          </div>
          <p className="text-muted-foreground mt-3 text-base">Store-aware retail news using your type, location, and weather.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => loadNews(query)} disabled={loading} className="px-4 py-2 rounded-xl bg-secondary text-foreground text-sm font-semibold flex items-center gap-2">
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />} Refresh
          </button>
          <button onClick={downloadPDF} className="px-4 py-2 rounded-xl bg-blue-500/10 text-blue-600 text-sm font-semibold flex items-center gap-2"><Download className="w-4 h-4" /> PDF</button>
        </div>
      </div>

      <div className="bg-card border border-blue-500/30 rounded-2xl p-4 shadow-[0_0_24px_rgba(59,130,246,0.12)]">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-secondary border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500/40"
            />
          </div>
          <button onClick={() => loadNews(query)} disabled={loading} className="px-5 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-card border border-blue-500/30 rounded-2xl p-5 shadow-[0_0_22px_rgba(59,130,246,0.16)]"><Store className="w-5 h-5 text-blue-500 mb-3" /><p className="font-bold text-lg">{store?.store_name || "Store"}</p><p className="text-sm text-muted-foreground">{store?.store_category || "Grocery & Supermarket"}</p></div>
        <div className="bg-card border border-pink-500/30 rounded-2xl p-5 shadow-[0_0_22px_rgba(236,72,153,0.14)]"><MapPin className="w-5 h-5 text-pink-500 mb-3" /><p className="font-bold text-base">{location || "Detecting location"}</p><p className="text-sm text-muted-foreground mt-1">Used for local news relevance</p></div>
        <div className="bg-card border border-orange-500/30 rounded-2xl p-5 shadow-[0_0_22px_rgba(249,115,22,0.14)]"><Cloud className="w-5 h-5 text-orange-500 mb-3" /><p className="font-bold text-lg">{weather ? `${weather.temp}C, ${weather.description}` : "Weather unavailable"}</p><p className="text-sm text-muted-foreground">Used for demand-sensitive signals</p></div>
      </div>

      {error && <div className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-500">{error}</div>}
      {loading ? (
        <div className="bg-card border border-border rounded-xl p-12 text-center"><Loader2 className="w-8 h-8 animate-spin text-blue-500 mx-auto mb-3" /><p className="text-muted-foreground">Fetching live news...</p></div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center justify-between rounded-2xl border border-blue-500/30 bg-blue-500/10 px-5 py-4 shadow-[0_0_24px_rgba(59,130,246,0.18)]">
            <h2 className="text-xl font-black text-foreground">Highlighted News Signals</h2>
            <span className="text-sm font-semibold text-blue-400">{allNews.length} cards</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {allNews.map((item, index) => {
              ...
              return (
              <a key={index} href={validLink} target={validLink ? "_blank" : undefined} rel={validLink ? "noopener noreferrer" : undefined} className={`group min-h-[220px] bg-card border border-blue-500/30 rounded-2xl p-6 shadow-[0_0_24px_rgba(59,130,246,0.14)] transition-all ${validLink ? "hover:shadow-[0_0_36px_rgba(59,130,246,0.28)] hover:border-blue-400/70 cursor-pointer" : "cursor-default opacity-90"}`}>
                {item.imageUrl && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={item.imageUrl} alt="" className="mb-4 h-36 w-full rounded-xl object-cover border border-blue-500/20 bg-secondary" loading="lazy" />
                )}
                <div className="mb-4 flex items-center justify-between">
                  <span className="rounded-full bg-blue-500/15 px-3 py-1 text-xs font-bold text-blue-400">NEWS #{index + 1}</span>
                  {validLink && <ExternalLink className="w-4 h-4 text-blue-400 opacity-70 group-hover:opacity-100" />}
                </div>
