...
export async function POST(request: Request) {
  try {
    const { products, userId, weather, weatherForecast, location, lang, extraPrompt } = await request.json();
...
Shopkeeper notes: ${extraPrompt?.trim() || "None"}
...
- If shopkeeper notes include local supplier limits, promotion timing, missing pack sizes, or extra context, reflect that in adjustmentReason and suggestions without inventing unavailable products.${langInstruction}`;
